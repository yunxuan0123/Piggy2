		...
		public static void PopulateObject(string value, object target, JsonSerializerSettings settings)
		{
			...
			using (JsonReader jsonTextReader = new JsonTextReader(new StringReader(value)))
			{
				...
				if (settings != null && settings.CheckAdditionalContent)
				{
					...
					{
						if (jsonTextReader.TokenType == JsonToken.Comment)
						{
							continue;
						}
						throw JsonSerializationException.Create(jsonTextReader, "Additional text found in JSON string after finishing deserializing object.");
					}
				}
			}
		}

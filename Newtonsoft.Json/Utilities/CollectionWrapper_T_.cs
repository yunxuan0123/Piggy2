...
{
	internal class CollectionWrapper<T> : IEnumerable<T>, ICollection<T>, IEnumerable, IList, ICollection, Interface1
	{
		...
		public object UnderlyingCollection
		{
			get
			{
				object obj = this._genericCollection;
				if (obj == null)
				{
					obj = this._list;
				}
		...
		public CollectionWrapper(IList list)
		{
			...
			ICollection<T> ts = list as ICollection<T>;
			ICollection<T> ts1 = ts;
			if (ts != null)
			{
				this._genericCollection = ts1;
				return;
			}
			this._list = list;
		}
		...
		{
			IEnumerable<T> ts = this._genericCollection;
			if (ts == null)
			{
				ts = this._list.Cast<T>();
			}
